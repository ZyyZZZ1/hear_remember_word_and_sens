...
- **修复实施**（2026-07-02）：
  - `main.py:1440`：调用处改传 `item["zh"]`（全量）✅
  - `_spelling_quiz_phase` 内部解耦（原 issues.md 说"零改动"，实测不行——因为函数内 R 重听 `main.py:694` 和 2 次错误 `main.py:727` 的 TTS 也用 `zh_text`，若不拆会念全量太冗长）：
    - `main.py:681`：新增 `zh_first = zh_variants[0] if zh_variants else zh_text`
    - `main.py:694`：R 重听 TTS 改用 `zh_first` ✅
    - `main.py:727`：2 次错误 TTS 改用 `zh_first` ✅
    - 屏幕展示（`main.py:724, 734`）保留 `zh_text` 全量——答错时显示所有释义有助于学习
  - 验证：`py_compile` 通过；逻辑追踪 `facultad` 用例确认 "系"/"学院"/"Fakultät(德语同源词)" 均判 ✓，TTS 只念 "(英语同源词faculty)"
- **提出时间**：2026-06-29
- **会话记录**：`c.markdown §5`
